...
public class AuthenticationController(IAuthService authService, ApplicationDbContext dbContext) : AbpController
{
    ...
    ///     Returns a BadRequest response with a message "Username or email is already taken." if the username or email is
    ///     already taken.
    ...
    public async Task<IActionResult> Register([FromBody] RegisterDto newUser)
    {
        ...
        return result ? Ok("User registered successfully.") : BadRequest("Username or email is already taken or role does not exist.");
    }
    ...
    ///     Returns an Unauthorized response with a message "User associated with refresh token not found." if the user
    ///     associated with the refresh token is not found.
    /// </returns>
    [HttpPost("refresh")]
